...
DWORD AllignSectionHeader(DWORD section_size, DWORD allignment, DWORD address)
{
	if (!(section_size % allignment))
		return address + section_size;

	return address + (section_size / allignment + 1) * allignment;
...
INT main(INT arg, PCHAR argv[])
{
	...
	//EthicalInjector.exe "section name" "target" "dll"
				...
				if (image_dos_header->e_magic == IMAGE_DOS_SIGNATURE)
				{
					...
					if (code_file != INVALID_HANDLE_VALUE)
					{
						...
						if (dll_size > 0)
						{
							...
							if (dll_buffer != ERROR)
							{
								DWORD returned_dll_byte, unused_bytes;
								BOOL dll_read = ReadFile(code_file, dll_buffer, dll_size, &returned_dll_byte, NULL);
								if (dll_read == TRUE && returned_dll_byte == dll_size)
								{
									...
									{
										image_section_header[PESections].Misc.VirtualSize = AllignSectionHeader(dll_size, image_optional_header->SectionAlignment, 0);
										image_section_header[PESections].VirtualAddress = AllignSectionHeader(image_section_header[PESections - 1].Misc.VirtualSize, image_optional_header->SectionAlignment, image_section_header[PESections - 1].VirtualAddress);

										image_section_header[PESections].SizeOfRawData = AllignSectionHeader(dll_size, image_optional_header->FileAlignment, 0);
										image_section_header[PESections].PointerToRawData = AllignSectionHeader(image_section_header[PESections - 1].SizeOfRawData, image_optional_header->FileAlignment, image_section_header[PESections - 1].PointerToRawData);
